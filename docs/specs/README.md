...
| [`DATE_FALLBACK.md`](./DATE_FALLBACK.md) — options-only author and issued-date fallback policies | Active | `citum-engine::processor::rendering::tests::date_fallback_options`, `citum-schema-style::bdd_scope_cascade` |
| [`DATE_SUBSTITUTE.md`](./DATE_SUBSTITUTE.md) — historical v1 identity-date policy | Superseded | Replaced by `DATE_FALLBACK.md` |
...
| [`TWO_NAME_DELIMITER_POLICY.md`](./TWO_NAME_DELIMITER_POLICY.md) — scoped contributor policy for two-name conjunction delimiters | Active | `bibliography.rs`, `citations.rs` |
