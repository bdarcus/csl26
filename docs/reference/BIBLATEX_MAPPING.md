...
> **Known to be stale.** Several rows below (`@collection`, `@mvcollection`,
> `@proceedings`, `@periodical`, `@reference`, `@inreference`) do not match
> what `crates/citum-refs/src/formats/biblatex/tables.rs` actually produces.
> `docs/schemas/type-map.json` (generated by `just schema-gen`) is the
> authoritative source until this file is replaced by a generated
> equivalent (bean csl26-veeg).
