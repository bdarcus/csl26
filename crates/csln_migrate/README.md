...
manually (human or agent-assisted), not generated by `csln-migrate` or
`infer-template.js`.
...
`csln-migrate` reads citation and bibliography templates from that file when
...
`csln-migrate` does not guarantee perfect output equivalence for every legacy
