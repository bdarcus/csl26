...
OUTPUT=$(cargo run --quiet --bin csln -- render refs -b tests/fixtures/references-expanded.json -s "$STYLE_FILE" -c tests/fixtures/citations-expanded.json --mode both --show-keys 2>/dev/null)
